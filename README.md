...
- desktop GUI: browser 기반 UI가 아니라 `PySide6` 데스크톱 앱입니다. raw scan, summarize, organize, lint 중심의 maintenance workflow와 graph 탐색, MCP-first agent 질문을 제공합니다.
...
오른쪽 Wiki Agent 패널은 MCP tool registry를 우선 사용하며, GUI에는 `agent route: mcp/codex`, `agent route: mcp/rule_based`, `agent route: direct fallback`처럼 현재 답변 경로가 표시됩니다.
