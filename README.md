...
- `gemini`: answer, ingest, review role에 연결되어 있습니다. concept role은 아직 연결하지 않았고 `rule_based` fallback을 사용합니다.
...
- `LLM_WIKI_GEMINI_COMMAND`: Gemini CLI 감지 및 answer/ingest/review 호출용 command입니다. 기본값은 `gemini`입니다.
...
- Gemini provider는 answer, ingest, review role에 연결되어 있습니다. concept role 연결은 이후 확장 경로입니다.
