...
따라서 desktop GUI는 파일 업로드 앱처럼 동작하지 않습니다. GUI는 PySide6 기반 3분할 화면에서 raw scan, summarize, organize, lint를 실행하고 결과를 보여주는 maintenance surface입니다.
...
- desktop GUI의 Wiki Agent 패널도 MCP tool registry를 우선 route로 사용하고, direct adapter 호출은 fallback으로만 남깁니다.
